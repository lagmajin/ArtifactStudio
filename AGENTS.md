...
`docs/` 配下の文書を新規作成または内容更新した場合は、文書冒頭に
`**最終更新:** YYYY-MM-DD` を記載・更新すること。既存文書に更新日欄がない場合は、今回の変更時に追加する。
ファイル名に日付が含まれていても、最終更新日の代わりにはしない。自動生成ファイルは生成日時の記載を優先し、手動で更新日を追記しない。
変更した文書だけを確認する場合は、`python tools/docs/audit_document_dates.py docs --file <path>` を使う。
...
### ラインエンディング（クロスプラットフォーム）

ソースファイルのリポジトリ上および作業ツリー上の改行コードは **LF (`\n`) に統一**する。Windows 固有の CRLF を正として扱わないこと。LF は Windows / macOS / Linux の各ツールチェーンで利用でき、OS 間のチェックアウトや差分で不要な全行変更を発生させない。

**ルール:**
- `.gitattributes` の `eol=lf` 指定を改変・回避しないこと。対象のソース拡張子を追加する場合も `eol=lf` を指定すること
- 既存ファイルの内容変更では、ファイル全体の改行コードを意図せず変換しないこと。変更後は差分に改行コードだけの大量変更がないことを確認すること
- 新規ファイルも LF とし、末尾には改行を 1 つ入れること
- `.ixx` / `.cppm` の変更は改行コード変更とモジュール依存変更を分離して確認すること。可能な限り `.ixx` を避け、`.cppm` 実装ファイルのみを変更すること
- 新規 `.cppm` / `.ixx` 追加は CMakeLists.txt 編集を伴うため全体再スキャン要因になる。既存ファイルへの追記・編集で代替可能か最初に検討すること

### モジュール / Qt 再発防止の簡易ルール

- PImpl の `Impl` 所有には `std::shared_ptr` / `std::unique_ptr` を新規採用しない。DLL 境界・C++20 module 境界でのアロケータ、ABI、デストラクタ実体の不一致を避けるため、原則として `Impl*` を明示所有する。所有クラスのデストラクタで `delete impl_; impl_ = nullptr;` を実行し、コピー／ムーブコンストラクタと代入演算子の扱い（禁止または深いコピー／所有権移譲）を必ず確認する。既存の `shared_ptr` / `unique_ptr` は互換性確認なしに機械的に置換しない。
...
`ReactiveEvents`（`ArtifactCore/include/Reactive/ReactiveEvents.ixx`、`Artifact/src/Service/ArtifactReactionExecutor*` を含む Reactive イベント駆動システム全体）は、ユーザーから明示的に依頼されるまで**一切触らないこと**。改善提案、拡張案、実装の継続、バグ修正の提案も含め、AI の側から話題にしない。このセクションに関する作業はすべてユーザーからの指示を待つ。

### 開発優先方針（2026-07-27）

当面は動画対応を後回しにし、以下の完成度向上を優先すること。
...
各対象では、編集操作、表示品質、変換、マスク、合成、保存・再読込、プレビュー安定性を優先する。動画読み込み・動画デコードなど開発期間の長い対応は、上記の基盤が十分に安定するまで新規実装や大規模な設計変更を後回しにすること。

### レンダラー更新の優先順位（2026-08-15）
...
ショートカットキーを追加・変更する場合は、Blender スタイルの汎用変換操作と矛盾させないこと。特に `G`（移動）、`R`（回転）、`S`（スケール）、`X` / `Y` / `Z`（軸拘束）、`Shift` / `Ctrl`（精密操作・スナップ等の修飾）、`Esc`（キャンセル）、`Enter` / 左クリック（確定）、右クリック（キャンセル）の組み合わせを予約された基本操作として先に確認する。既存の `ShortcutBindings`、アクティブなワークスペース／ツールのコンテキスト、入力フォーカスとの衝突を調査し、単一キーを場当たり的にハードコードしない。Blender 系操作と AE 系ツール切替が競合する場合は、ユーザーが明示した方式または設定可能なコンテキスト別バインドを優先し、暗黙に上書きしない。

### Dock タブとドキュメントタブの責務分離

`Composition Viewer`、`Project`、`Inspector` などは Dock／パネルタブとして扱い、表示・配置・分離・Dock 復帰・最大化・閉じるなど、ワークスペース管理だけを所有する。未保存状態、保存、複製、ドキュメント固有の「他を閉じる／右側を閉じる」などを Dock タブへ持ち込まない。

`Comp1`、Layer Solo の編集対象などはドキュメント／編集コンテキストタブとして扱い、未保存状態とドキュメントライフサイクルを所有する。未保存表示はタブ名の近くに小さな状態マークで示し、Dock タブ名には表示しない。ドキュメントタブを閉じる操作は、既存の dirty-state 判定と保存確認を必ず経由し、タブ UI が独自に破棄しない。

タブに常時表示する操作は原則としてタブ名と閉じるボタンだけに限定する。未保存・読み取り専用・処理中などの状態表示は必要時だけ追加し、ピン、分離、最大化などは右クリックメニューまたはタブ一覧へ置く。右クリックメニューも、Dock タブではワークスペース操作、ドキュメントタブではドキュメント操作に限定し、両者の項目を混在させない。

#### 固定キー禁止・ローカルバインド必須

- ユーザーが起動、切替、確定、キャンセル、リセット、表示変更などの操作として認識するキーボード入力は、原則としてバインドなしの固定キーにしない。最低でも `ShortcutBindings` に専用 `ShortcutId`、既定キー、表示名、永続化キー、所有コンテキストを登録し、対象ウィジェットは `ShortcutBindings::matches()` で判定する。
- アプリ全体で必要な操作だけを `Global` とし、Viewport、Timeline、Inspector、Project、各モーダル操作はそれぞれのローカルコンテキストに所属させる。フォーカス中の所有サーフェスだけがローカルバインドを解決し、他のパネルからキーを奪わないこと。
- `event->key() == Qt::Key_*` による操作コマンドの新規ハードコード、および `ShortcutBindings::matches(...) || event->key() == ...` のような固定キーフォールバックは禁止する。既定キーは `ShortcutBindings::resetToDefaults()` だけを正規の定義元とする。
- モーダル操作の `X` / `Y` / `Z`、`Esc`、`Enter` なども、ユーザーが変更可能な操作である限り `Modal.<機能名>` コンテキストのバインドとして定義する。モーダル優先順位は維持するが、キーそのものをイベントハンドラへ直書きしない。
- 例外は、テキスト入力欄へ渡す文字入力、数値入力の数字・小数点・符号、IME／OS予約キー、Qtの標準テキスト編集イベント、デバッグ専用で製品操作に露出しない入力に限定する。例外を追加する場合は、固定キーが必要な理由と変更不能でよい理由をコードコメントおよび最終報告へ記載する。
- 既存の固定キーを含む入力処理へ機能変更を加える場合は、今回触る操作を同時にローカルバインドへ移行する。無関係な既存固定キーまで一括変更して作業範囲を広げず、残存箇所は必要に応じて `Insight.md` に記録する。
- 実装後は、設定画面への表示、保存／再読込、空バインドによる無効化、同一コンテキスト内の競合検出、別コンテキストで同じキーを使えること、対象外フォーカスでは発火しないことを確認する。
