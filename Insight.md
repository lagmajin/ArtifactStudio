...
- **対応・確認:** `Artifact.Layer.Abstract.Utilities` へ有限値clamp処理を切り出し、`QDebug` / `qWarning` のストリーム演算子を可変長書式のログ呼び出しへ置換した。MSVC 14.52 Preview と、障害を再現していた MSVC 14.51.36231 の双方で `ArtifactAbstractLayer.cppm` のオブジェクト生成が成功した。
