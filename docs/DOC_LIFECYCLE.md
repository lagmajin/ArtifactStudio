# ドキュメントライフサイクルルール

**最終更新:** 2026-08-05

## 最終更新日の記録

`docs/` 配下の文書は、新規作成時および内容を更新した時に、文書冒頭へ次の形式で最終更新日を記載する。

```markdown
**最終更新:** YYYY-MM-DD
```

- 更新日は実際に内容を変更した日付とする。
- ファイル名に日付が含まれていても、最終更新日の代わりにはならない。
- 既存文書に更新日欄がない場合は、次回の内容変更時に追加する。
- 自動生成文書は、生成処理が出力する生成日時を優先し、手動の更新日欄は追加しない。
- この日付を使って、長期間更新されていない文書の棚卸し・アーカイブ候補の抽出を行う。

棚卸しはビルド不要の監査スクリプトで実行できる。

```powershell
python tools/docs/audit_document_dates.py docs --as-of 2026-08-05 --max-age-days 180
```

`--max-age-days` を省略すると、日付の欠落・形式不正・未来日だけを検出する。
変更した文書だけを確認する場合は、`--file docs/planned/example.md` を追加する（複数指定可）。

## 文書の種類と配置

| ディレクトリ | 内容 | ライフサイクル |
|-------------|------|--------------|
| `docs/planned/` | 計画中のマイルストーン | Draft → In Progress → Complete 後は `done/` へ |
| `docs/done/` | 完了したマイルストーン | 6ヶ月以上更新がない場合 `archived/` へ |
| `docs/analysis/` | 分析レポート | 更新が必要になったら上書き、旧版は削除 |
| `docs/technical/` | 技術詳細・設計仕様 | コード変更に合わせて更新 |
| `docs/bugs/` | バグレポート・分析 | 修正確認後も参照用に残す |
| `docs/shared/` | AI 共有メモ | 新しい AI セッションのための引継ぎ |
| `docs/worklog/` | 作業ログ | 作業完了後も履歴として残す |

## ステータス行のルール

`docs/planned/` の全マイルストーン文書は、冒頭に以下の形式でステータスを持つこと：

```
**ステータス:** Not Started / In Progress / Blocked / Complete
```

完了時はファイルを `docs/planned/` から `docs/done/` へ移動し、冒頭に追加する。

**ステータス:** ✅ Complete

## 自動チェック

`tools/generate_doc_inventory.py` は、`docs/INDEX_GENERATED.md` の生成に加えて次を警告する。

- `docs/planned/` に残っている `Complete` 文書
- Markdown 内の絶対パスリンク
- 壊れている相対リンク

運用時は次のように使う。

```powershell
python tools/generate_doc_inventory.py
```
