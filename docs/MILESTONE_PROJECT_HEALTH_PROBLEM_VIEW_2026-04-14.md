...
### Phase 2.5: Pre-Render Validation Hook

**目的:**
レンダー開始直前に検証を行い、無意味なレンダリング（失敗、長時間放置）を防ぐ。

**作業項目:**
- **レンダー用コンテキストの定義**
  - `DiagnosticEngine::validate(project, Context::RenderQueue)` のような呼び出し経路
- **ブロック条件（Error）**
  - ソースファイルの欠落（Missing File）
  - マット参照の欠落（Missing Matte）
  - 循環依存（Circular Dependency）
  - 出力パスの不正（Invalid Output Path）
  - **挙動:** ダイアログでエラー一覧を表示し、レンダーを自動キャンセル
- **警告条件（Warning）**
  - 高解像度（4K超えなど）によるメモリ逼迫の懸念
  - アクティブなレイヤーが存在しない
  - **挙動:** 「警告を無視して続行しますか？」の確認ダイアログ

**完了条件:**
- レンダーキュー開始時にバリデーションが走る
- エラーがある場合は自動停止する
...
- [`docs/done/MILESTONE_RENDER_PREFLIGHT_2026-06-02.md`](./done/MILESTONE_RENDER_PREFLIGHT_2026-06-02.md)
