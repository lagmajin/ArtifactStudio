# Insight Archive (through 2026-08-11)

**最終更新:** 2026-08-11

> `Insight.md` に記録されていた、実装済みの局所修正と履歴の保存版。現行の未解決事項はルート `Insight.md` を参照。
...
### 2026-08-10 — Audio backend の PCM→device 変換境界
...
## 2026-08-10: AudioSpectrum の FFT ビン数上限

- **関連:** `ArtifactCore/include/Audio/AudioSpectrum.ixx`, `ArtifactCore/src/Audio/AudioSpectrum.cppm`
- **事実:** `setBins()` は正数化のみを行い、`process()` は設定値をそのまま `spectrum_.resize()` に渡していた。
- **仮説:** 外部設定や壊れた保存値が極端に大きい場合、スペクトラム計算前に過大なメモリ確保を試みる可能性がある。未検証。
...
## 2026-08-10: Core Parametric EQ JSON バンド数上限

- **関連:** `ArtifactCore/include/Audio/AudioParametricEQ.ixx`, `ArtifactCore/src/Audio/AudioParametricEQ.cppm`
- **事実:** 通常の `setBandCount()` は最大 64 バンドに制限していたが、`fromJson()` は JSON 配列全件を `reserve()` していた。
- **仮説:** 壊れた／悪意ある保存データの大量バンド配列が、不要なメモリ確保とロード遅延を起こす可能性がある。未検証。
- **対応:** JSON 復元も最大 64 件だけを読み込み、公開 API のバンド数上限と統一した。
- **価値／懸念:** 設定経路による上限差をなくす。64 件を超える保存済みバンドは読み込み時に切り捨てられる。
- **次に確認:** ビルド時に 0／64／64 超バンドの JSON 往復を確認する。

## 2026-08-10: Artifact 波形表示点数の整数境界

- **関連:** `Artifact/src/Audio/ArtifactAudioWaveform.cppm`
- **事実:** 出力幅は入力サンプル数以下に制限されていたが、表示点数の `outputWidth * 2` は `int` のまま計算されていた。
- **仮説:** 極端に大きい入力サンプル数で reserve 引数がオーバーフローし、誤った容量要求になる可能性がある。未検証。
...
## 2026-08-10: AudioPreview 再生開始状態の整合

- **関連:** `Artifact/src/Widgets/AudioPreviewWidget.cppm`
- **事実:** `AudioPlaybackEngine::play()` は renderer の open／start 結果を確認せず、失敗時も再生フラグと timer を開始していた。空データ時の `setPosition()` は clamp 上限が負値になり得た。
- **仮説:** オーディオデバイスが利用できない環境で UI が再生中表示のまま進み、位置通知も負値になる可能性がある。未検証。
- **対応:** open 成功と `isActive()` を確認してから再生状態へ遷移し、失敗時は device を閉じる。空データ位置は 0 に固定した。
- **価値／懸念:** UI の再生状態と実デバイス状態を一致させる。デバイスが一時的に利用できない場合、再生要求は無音状態で終了する。
- **次に確認:** 実機またはビルド時に open／start 失敗と空データ seek を確認する。

## 2026-08-10: AudioPreview プリロード位置境界

- **関連:** `Artifact/src/Widgets/AudioPreviewWidget.cppm`
- **事実:** プリロード済みセグメントの総フレーム数を `int` へ無検証加算し、feed counter は `static` で全 AudioPlaybackEngine インスタンス間に共有されていた。
- **仮説:** 長時間音声で総数や現在位置が overflow し、複数 preview の再生タイミングが互いに影響する可能性がある。未検証。
- **対応:** 総数と現在位置の加算を `int` 最大値で飽和させ、位置探索は `qint64` 累積に変更した。feed counter はインスタンスメンバー化し、load／stop でリセットする。
- **価値／懸念:** 長時間素材と複数 preview の状態を分離する。`int` 公開位置 API の上限自体は維持される。
- **次に確認:** ビルド時に長時間素材の位置境界と複数 preview の feed 周期を確認する。

## 2026-08-10: AudioPreview 音量入力の有限値境界

- **関連:** `Artifact/src/Widgets/AudioPreviewWidget.cppm`
- **事実:** `setVolume()` は入力を直接 `std::clamp` し、`volumeToDb()` は非有限値を `log10()` へ渡し得た。
- **仮説:** 設定値や UI 連携から NaN／∞ が入ると、renderer の master volume と表示状態に非有限値が伝播する可能性がある。未検証。
- **対応:** 非有限音量は既定値 1.0 に戻し、dB 変換では無音値 -144 dB として扱う。
...
- **関連:** `Artifact/src/Widgets/AudioPreviewWidget.cppm`
- **事実:** 外側の click／公開 `play()` は内部 play の成否を確認せず再生開始通知を出していた。公開 `setVolume()` は未検証値を slider の int へ変換し、waveform view は非正 sample rate を保持できた。
- **仮説:** デバイス失敗時の誤った UI 状態、slider 値の範囲外／変換異常、時間表示の不整合が起きる可能性がある。未検証。
...
## 2026-08-10: FFmpeg seek の整数化上限

- **関連:** `ArtifactCore/src/Codec/FFMpegAudioDecoder.cppm`
- **事実:** 有限かつ非負の seek 秒数でも、timestamp 用 `int64_t` と seek frame 用 `qint64` の両方へ変換していた。
- **仮説:** 極端な有限値で double→整数変換が範囲外になり、未定義／不正な timestamp や frame 位置になる可能性がある。未検証。
...
## 2026-08-10: AudioSyncTools align index 境界

- **関連:** `Artifact/src/Audio/ArtifactAudioWaveform.cppm`
- **事実:** 相関計算の `i + lag` が int のまま計算され、範囲判定より前に加算されていた。
- **仮説:** 大きな音声配列と正の lag の組み合わせで int overflow が起き、誤った添字または未定義動作につながる可能性がある。未検証。
...
## 2026-08-10: AudioDelay ブロック連続性

- **関連:** `ArtifactCore/include/Audio/AudioDelay.ixx`, `ArtifactCore/src/Audio/AudioDelay.cppm`
- **事実:** ディレイ処理のバッファ index が各 `process()` 呼び出しで `i % bufSize` に戻っていた。
- **仮説:** ストリーミング音声を複数ブロックに分けて処理すると、ブロック境界で遅延線が再生位置を失い、連続したエコーにならない可能性がある。未検証。
- **対応:** チャンネルごとの書き込み位置をインスタンス状態として保持し、バッファ拡張時だけ位置をリセットする。
- **価値／懸念:** AudioChorus と同様に、AudioDelay のブロック間状態を維持する。パラメータ変更時の既存バッファ扱いは従来どおりで、音色リセットまでは行わない。
- **次に確認:** ビルド時に1ブロック／複数ブロック、ディレイ時間境界、左右チャンネル独立性を確認する。

## 2026-08-10: AudioPanner channel data boundary

- **関連:** `ArtifactCore/src/Audio/AudioPanner.cppm`
- **事実:** `applyPanning()` は `channelCount()` と gain 配列長だけを条件に `segment.channelData[ch]` へアクセスしていた。
- **仮説:** メタデータ上のチャンネル数が実データ配列数を超える不整合時に、チャンネル配列を範囲外参照する可能性がある。未検証。
...
## 2026-08-10: AudioTone shared phase progression

- **関連:** `ArtifactCore/src/Audio/AudioTone.cppm`
- **事実:** `phase_` はチャンネル内のサンプル処理ごとに進められていた。
- **仮説:** ステレオ入力では右チャンネルのトーンが左チャンネルの処理後から始まり、フレーム数ぶん位相ずれする可能性がある。未検証。
- **対応:** 位相増分を計算し、最初の処理チャンネルで各フレームにつき1回だけ進める。
- **価値／懸念:** チャンネル間で同じトーン位相を保つ。Noise 波形のチャンネルごとの乱数生成は維持する。
- **次に確認:** ビルド時に mono／stereo、複数ブロック、正弦波と square 波の左右位相を確認する。

## 2026-08-10: AudioBassTreble filter state continuity

- **関連:** `ArtifactCore/include/Audio/AudioBassTreble.ixx`, `ArtifactCore/src/Audio/AudioBassTreble.cppm`
- **事実:** low/high IIR state が `process()` 内ローカル変数で、各ブロックの先頭サンプルから再初期化されていた。
- **仮説:** ブロック境界でシェルフフィルターが毎回リセットされ、連続音声に不要な過渡応答が発生する可能性がある。未検証。
...
## 2026-08-10: Diligent buffer-map failure guards

- **関連:** `Artifact/src/Render/DiligentImmediateSubmitter.cppm`, `Artifact/src/Render/DiligentBindlessSubmitter.cppm`
- **事実:** 共通 `mapWriteDiscard` helper は context／buffer／data と `MapBuffer` の mapped pointer を確認せず `memcpy` していた。
- **仮説:** device loss、破棄済み buffer、または map failure 時に null pointer copy で render thread がクラッシュする可能性がある。未検証。
