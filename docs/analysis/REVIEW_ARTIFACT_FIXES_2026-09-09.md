...
| ID | 基準 | 状態 |
|---|---|---|
| ACC-001 | 現mainにない変更をコミット単位で特定できる | 完了 |
| ACC-002 | 各候補について現行API・責務・Undo経路との整合を確認できる | 完了（採用／保留を分類） |
| ACC-003 | 採用候補にビルド・実機確認が必要な点を明記できる | 完了（未実行） |
| ACC-004 | 却下・保留理由をコミットとファイルへ追跡できる | 完了 |
...
- Composition cycling は旧実装が新規 `QShortcut` と signal 接続に依存するため、現行のショートカット／ActionManager 経路へ再設計してから導入する。
