...
- `MediaPlaybackController::FFmpegPlaybackBackend::open()` で video / audio decoder 初期化失敗を `false` に反映するようにした
- `MediaImageFrameDecoder::initialize()` で `avcodec_open2()` と `sws_getContext()` の失敗理由を出力するようにした
- video stream が無い場合は `open()` を失敗扱いにして、`lastError` に理由を残すようにした
...
- packet wait の上限を固定値から、ファイルサイズに応じて増える値へ変更した
- video / audio の待ちループで同じ上限を共有するようにした
...
- `MediaSource::seek()` で `av_seek_frame()` 失敗時に `avformat_seek_file()` を retry するようにした
- どちらの seek も失敗した場合は理由をログに出すようにした
...
- ここはまだ未修正
- `ArtifactVideoLayer::loadFromPath()` 側で `firstFrame` の成功失敗に応じて `isLoaded_` を遅らせるのが次の手
...
- ここはまだ未修正
- まずは open / seek / decode の成功境界を締め、再現頻度を下げるのを優先した
...
- ここはまだ未修正
- 現時点では getNextVideoFrame() 側の待ち上限を file size ベースに広げるのみ
...
- `Failed to initialize video decoder` だけでなく、`video decoder initialization failed` / `audio decoder initialization failed` / `No video stream found` が出たら open failure 扱い
- `seek` 失敗は `av_seek_frame` と `avformat_seek_file` の両方の失敗理由がログに出る
- packet wait は固定 100 回ではなく、ファイルサイズに応じた上限に拡張される
