...
![Add Effect 改訂モック](add-effect-dialog-refined-2026-09-25.png)

ユーザーが選択した第2案を基にした視覚参照。左のアイコン付きカテゴリ表示を維持し、検索を結果一覧の上へ移し、中央の一覧を広げ、右の詳細を簡潔に整理する。画像内の効果名・説明・件数はレイアウト確認用であり、実装では既存の `EffectCatalogEntry` を表示元とする。検索、カテゴリ絞り込み、選択、ダブルクリック追加、ステージフィルター、確定／キャンセルの挙動は変更しない。下記の旧モックは比較用に残す。
