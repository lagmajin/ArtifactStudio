...
## 現行画面からの追従調整（2026-09-07）

- owner-drawラベルが `QLabel::alignment()` を無視していた問題を修正し、空状態のタイトルと説明を実際に中央配置。
- 対象レイヤーをレイヤー名と `Layer effects` の二段表示にし、ヘッダーの高さと余白をモックへ近づけた。
- Add Effect、検索欄、空状態アイコンの寸法と間隔を拡大し、操作帯としてのまとまりを強化。
- Plug-insフッターを48pxへ広げ、区切り線とowner-drawの設定記号を追加。

既存のエフェクト追加・検索・並べ替え・有効切替・Undo経路は変更していない。QtCSS、新規シグナル接続は追加していない。ビルド・実画面検証は未実施。
