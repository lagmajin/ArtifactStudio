"""Project Open Picker v3 mockups (concept).

Ver3 = Minimal-delta: keep the 3-panel skeleton adopted in the
2026-09-12 DCC concept, but close the *representation gap* between the mock
and the current runtime by replacing the single generic icon per recent project
with a real composition thumbnail. No layout restructure.

Outputs
  project-open-v3-candidate-2026-09-26.png
  project-open-v3-before-after-2026-09-26.png   (current runtime vs v3)
"""
...
def rr(d, box, radius=4, fill=PANEL_2, outline=LINE, width=1):
...
def thumbnail(d, box, kind):
    ...
    if kind == 'lunar':
        ...
        d.rectangle((x0, y0, x1, y1), fill=(48, 52, 64))
        for cx, r in [(w * 0.24, 34), (w * 0.52, 42), (w * 0.78, 30)]:
            d.ellipse((x0 + cx - r, y0 + h * 0.30 - r, x0 + cx + r, y0 + h * 0.30 + r),
                      fill=(64, 68, 80))
        for i in range(4):
            d.ellipse((x0 + cx - 8 + i * 10, y0 + h * 0.72, x0 + cx + 8 + i * 10,
                       y0 + h * 0.80), fill=(156, 158, 164))
    elif kind == 'circuit':
        d.rectangle((x0, y0, x1, y1), fill=(38, 34, 40))
        for i in range(3):
            d.line((x0 + 20, y0 + h * (0.2 + i * 0.26), x1 - 20,
                    y0 + h * (0.24 + i * 0.26)), fill=(150, 220, 235), width=2)
        for i in range(2):
            d.line((x0 + w * 0.3 + i * w * 0.4, y0 + 20, x0 + w * 0.3 + i * w * 0.4,
                    y1 - 20), fill=(150, 220, 235), width=1)
...
PROJECTS = [
    ...
    dict(name='Archive_2025', path=r'D:\Projects\Client Work\Archive_2025',
         ...
         version='2', comps='31', assets='204', status='Missing sources',
         health_color=AMBER, warn='3 external sources', favorites=False, sel=False),
    dict(name='R&D', path=r'D:\Projects\Client Work\R&D',
         thumb='circuit', opened='2025-04-12  13:27', res='1920 x 1080', dur='00:01:05',
    ...
    dict(name='Environment Test', path=r'D:\Projects\Client Work\Environment Test',
         thumb='cloud', opened='2025-03-30  10:09', res='3840 x 2160', dur='00:00:20',
...
def nav_toolbar(d, y, w):
    ...
    for label in ['D:\\', 'Projects', 'Client Work']:
        ...
        crx += len(label) * 7 + 14
        d.line([(crx - 2, cy - 3), (crx + 2, cy), (crx - 2, cy + 3)], fill=FAINT, width=1)
        crx += 8
    rr(d, (crx, cy - 16, 520, cy + 16), 4, (21, 23, 27), (96, 102, 110))
    txt(d, (crx + 24, cy), 'Project Open v3', FAINT, F12, 'lm')
    d.line((534, cy - 12, 534, cy + 12), fill=LINE)
    search_x1 = w - 24
    rr(d, (search_x1 - 236, cy - 16, search_x1, cy + 16), 4, (21, 23, 27), (96, 102, 110))
    d.ellipse((search_x1 - 222, cy - 4, search_x1 - 216, cy + 2), outline=FAINT, width=1)
    d.line((search_x1 - 216, cy + 2, search_x1 - 212, cy + 6), fill=FAINT, width=1)
    txt(d, (search_x1 - 200, cy), 'Search projects…', FAINT, F12, 'lm')
...
def sidebar(d, x0, y0, x1, y1, active_label):
    ...
    for glyph, label, on in PLACES:
        ...
        if glyph == 'star':
            d.line(star_pts(cxx, cyy, 7, 3) + [star_pts(cxx, cyy, 7, 3)[0]],
                   fill=YELLOW if label == 'Favorites' else tint, width=1)
        elif glyph == 'clock':
        ...
        txt(d, (x0 + 50, cyy), label,
            ...
            B11 if (on or label == active_label) else F11, 'lm')
        y += 36


def project_tile(d, box, p):
    x0, y0, x1, y1 = box
    sel = p['sel']
    d.rounded_rectangle((x0, y0, x1, y1), 6,
                        fill=(32, 45, 68) if sel else PANEL,
    ...
    if p['favorites']:
        sx, sy = x1 - 28, y0 + 24
        d.line(star_pts(sx, sy, 7, 3) + [star_pts(sx, sy, 7, 3)[0]], fill=YELLOW, width=1)
    ty = y0 + 122
    txt(d, (x0 + 12, ty), p['name'], TEXT if sel else DIM, B12 if sel else F12, 'lm')
    txt(d, (x0 + 12, ty + 20), p['path'], FAINT, F10, 'lm')
    txt(d, ((x0 + x1) // 2, ty + 34), p['res'], DIM, F10, 'mm')
    txt(d, (x1 - 12, ty + 34), p['dur'], DIM, F10, 'rm')
    if p['warn']:
        ay = ty + 56
        d.polygon([(x0 + 16, ay), (x0 + 22, ay + 12), (x0 + 10, ay + 12)],
                  outline=AMBER, width=1)
        txt(d, (x0 + 28, ay + 6), p['warn'], AMBER, F10, 'lm')
    else:
        ay = ty + 56
    ay += 18
...
def inspector(d, x0, y0, x1, y1, p):
    ...
    d.line((px, ty - 14, x1 - 20, ty - 14), fill=(38, 42, 47))
    if p['warn']:
        d.polygon([(px, ty - 2), (px + 6, ty + 8), (px - 6, ty + 8)], outline=AMBER, width=1)
        txt(d, (px + 12, ty + 6), p['warn'], AMBER, F10, 'lm')
        ty += 20
    ay = ty + 6
    rr(d, (px, ay, px + 14, ay + 14), 3, fill=(38, 58, 74), outline=ICE)
    d.line([(px + 3, ay + 7), (px + 6, ay + 10)], fill=ICE, width=2)
    d.line([(px + 6, ay + 10), (px + 11, ay + 4)], fill=ICE, width=2)
    txt(d, (px + 20, ay + 7), 'Open last composition', DIM, F12, 'lm')
    ay += 26
    on2 = bool(p['warn'])
    if on2:
        rr(d, (px, ay, px + 14, ay + 14), 3, fill=(44, 38, 30), outline=AMBER)
        d.line([(px + 3, ay + 7), (px + 6, ay + 10)], fill=AMBER, width=2)
        d.line([(px + 6, ay + 10), (px + 11, ay + 4)], fill=AMBER, width=2)
    else:
        rr(d, (px, ay, px + 14, ay + 14), 3, fill=(26, 29, 33), outline=(78, 84, 92))
    txt(d, (px + 20, ay + 7), 'Validate missing sources after open', DIM, F12, 'lm')
...
def build_before_after(path):
    ...
    im.paste(cur, (0, 46))
    v3 = grad((half_w, panel_h), (22, 24, 28), (14, 15, 18))
    d3 = ImageDraw.Draw(v3)
    window_chrome(d3, 'Open Project', half_w)
    nav_toolbar(d3, 40, half_w)
    d3.rectangle((0, 86, half_w, panel_h - 8), fill=(20, 23, 26))
    side_x = 178
    insp_x = half_w - 356
    d3.rectangle((side_x, 86, insp_x, panel_h - 8), fill=(19, 22, 25))
    sidebar(d3, 0, 86, side_x, panel_h - 8, 'Recent Projects')
    txt(d3, (side_x + 20, 108), 'Recent Projects', TEXT, B13, 'lm')
    txt(d3, (insp_x - 20, 108), '6 projects', MUTED, F11, 'rm')
    ...
    for i, p in enumerate(PROJECTS):
        ...
        if by + tile_h > panel_h - 24:
            break
        project_tile(d3, (bx, by, bx + tile_w, by + tile_h), p)
    inspector(d3, insp_x, 86, half_w, panel_h - 8, PROJECTS[0])
    footer(d3, panel_h - 60, half_w, panel_h - 8, '6 projects')
    im.paste(v3, (0, 46 + panel_h + 38))
    txt(d, (16, 22), 'CURRENT  -  generic icon per project, no preview, no '
                     'resolution, no health, no counts', RED, F12, 'lm')
    txt(d, (16, panel_h + 68), 'VER3  -  same skeleton, real preview thumbnails '
                               'replacing the generic icon, plus health / counts / '
                               'external-source warning in the inspector', GREEN, F12, 'lm')
    d.line((0, panel_h + 46, half_w, panel_h + 46), fill=LINE)
    im.save(path)


if __name__ == '__main__':
    build_candidate(OUT / 'project-open-v3-candidate-2026-09-26.png')
    build_before_after(OUT / 'project-open-v3-before-after-2026-09-26.png')
    print('project open v3 mockups written to', OUT)
