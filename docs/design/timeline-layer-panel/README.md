...
通常タイムライン全体の外観は、2026-09-13承認の[通常タイムライン採用デザイン](../timeline/README.md)を優先する。本ページの画像と実装記録は以前の左ペイン採用案として保持する。
...
- 数値・真偽値は値欄のクリックで専用ダイアログから編集。数値は値の上を左右にドラッグして調整でき、移動量がドラッグ開始距離未満ならダイアログを開く。静的値は既存のプロパティUndo、既存キーは接線・anchorを維持したキースナップショットUndo、キー間の編集は既存モデルによる現在フレームへのキー追加を利用。ロック中は編集しない。
...
標準幅・狭幅・高DPIでの列整列、列幅変更・列非表示、長い名前、Variant・マット表示、値クリック編集・左右ドラッグ調整とUndo/Redo、キー間編集、ロック状態、スクロール時のヒット位置、右ペインとの行同期を確認する。値編集は[2026-09-25の採用モック](../timeline-value-edit-dialog/timeline-value-edit-dialog-concept-2026-09-25.png)に沿う専用ダイアログを使う。新規SVGのリソース反映は次回のユーザー指示によるビルド時に確認する。
