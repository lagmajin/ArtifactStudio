...
## 改善Verモック（2026-09-26・概念・実装仕様の確定ではない）

本フォルダに、上の採用モックと並べて比較するための**改善Verを3案**生成した。いずれも**採用ではなく概念であり、実装仕様の確定ではない**。既存の承認済み画像（`approved-normal-timeline-2026-09-13.png` ほか）は読み取り参照のみで、上書き・リサイズ・削除は行っていない。

![改善VerA スリムレール](timeline-improve-a-slim-rail-2026-09-26.png)

![改善VerB プロパティ列分割](timeline-improve-b-split-column-2026-09-26.png)

![改善VerC 固定フッターストリップ](timeline-improve-c-footer-strip-2026-09-26.png)

- A（slim rail / value chip）: 左ペインを 268px まで狭め、Value をレイヤー行ではなくクリップ上のチップへ移す。要した余白の最小化意図。Transform 行は従来どおり選択レイヤーの下にインライン展開。
- B（property column split）: 左ペインを 340px まで広げ、Layer / Value / Parent / Blend を列として明示。値の常時可視化と列対応の確認を意図。
- C（single row + footer strip）: 1レイヤー1行に固定し、Transform プロパティを画面下部の固定ストリップへ集約。キー数のサマリを左ペインに残す。

判断ポイント: レイヤー行に常時表示する情報の量、Value を常に見せるか、Transform の編集面をインラインで残すか下部固定へ逃がすか。

生成スクリプト: `generate_timeline_improvement_mockups.py`（Pillow による手続き描画。外部API・ネットワーク不使用。既存の承認画像を読み書きしない）。

```powershell
cd docs/design/timeline; python generate_timeline_improvement_mockups.py
```

- 3案は同一サイズ 1832×858、同一配色・同一フォント・同一レイヤー定義で描画しており、差分はペイン構成と配置のみ。
- cache bar／work area／playhead／navigator／transport の描画は共通関数を共有。キャンバス内仕様（ギズモ、描画経路、入力）が変わる提案は含まない。
- 採用モックの全体の外観は `approved-normal-timeline-2026-09-13.png` を優先する。改善Ver画像を理由に新しいショートカット・ボタン・タイムライン構造を導入しない。
- 実機実装・ビルド・DPI確認は未実施。あくまでレイアウト比較用の静的概念図である。
