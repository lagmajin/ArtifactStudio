...
- **ルール**: コンポジション領域は背景色（`backgroundColor`）で塗りつぶされるが、 **レイヤーブレンド計算には一切影響しない**
  - 背景色は単なる初期クリアカラーであり、Compositing パイプラインの入力来源ではない
  - レイヤー同士の Alpha Blending では、背景色は「存在しない」ものとして扱う
  - 最終的な表示結果には背景色が明示的に描画されるが、レイヤー間の色計算には関与しない
- **実装上の意図**:
  - GPU パイプライン: `accumRTV` を `bgColor` で Clear 後、各レイヤーを順次 Blend
  - CPU フォールバック: 同様に `renderer_->clear(bgColor)` で初期化後、レイヤー描画
- **禁止事項**:
  - 背景色を「不透明な事前レイヤー」として扱う拡張機能（例: 背景をアルファブレンド対象に含める）は許可されていない
