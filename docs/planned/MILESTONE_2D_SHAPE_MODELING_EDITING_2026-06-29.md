...
- `ArtifactRenderLayerWidgetV2` の context menu で path editing の tangents 数も見えるようにした
- `ArtifactRenderLayerWidgetV2` の context menu で polygon state を確認しやすくした
- `ArtifactRenderLayerWidgetV2` の context menu で polygon editing の segment 数も見えるようにした
...
- `ArtifactRenderLayerWidgetV2` の state 表示に selected vertex も出すようにした
- `EditMode::Shape` を追加して、Shape tool を独立した編集モードとして扱えるようにした
- `ArtifactRenderLayerEditor` の Edit Mode メニューを `Shape` に合わせて更新した
- `ArtifactToolService` で `Shape` モードを `ToolType::Shape` に正しく結び直した
- `ArtifactRenderLayerWidgetV2` の shape overlay / context routing を `Shape` モードでも通るようにした
- `ArtifactRenderLayerWidgetV2` に shape hover HUD を追加して、vertex / segment の現在位置を見やすくした
- `ArtifactRenderLayerWidgetV2` の shape context menu を `Editable Path` 文言に合わせた
- `ArtifactRenderLayerWidgetV2` の shape selection grammar を `Shape` mode 前提で追い込み始めた
- `ArtifactRenderLayerWidgetV2` の path tangent drag で `Alt` による独立編集を通せるようにした
- `ArtifactRenderLayerWidgetV2` の polygon/path conversion 後に先頭要素を軽く選択状態へ戻すようにした
- `ArtifactRenderLayerWidgetV2` の vertex / tangent HUD に selected 状態を出して、掴み中の要素を明示するようにした
- `ArtifactRenderLayerWidgetV2` の polygon/path conversion を undo/redo 対応の変換コマンドにした
- `ArtifactRenderLayerWidgetV2` の undo ラベルを `Edit Polygon` / `Edit Path Vertices` 寄りに整えた
