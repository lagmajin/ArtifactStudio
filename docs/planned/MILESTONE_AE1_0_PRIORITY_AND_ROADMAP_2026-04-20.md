...
## 2026-08-15 現行優先方針との照合

- 1.0 の必須項目は、Host／Context／ROI、Render Queue、Diagnostics、Mask／Shape、Keyframe、Motion Blur、Workspace の個別基盤が進んでいるが、E2E 完了ではない。
- 現行の開発優先方針（静止画、連番画像、Shape、画像処理／合成、3D）と整合する部分は、1-5 の合成・Shape、1-3 の静止画／連番出力、1-7 の品質確認を先に扱う。
- 動画対応を後回しにする方針により、旧ロードマップの video／proxy を短期最優先とは扱わない。各専用マイルストーンの現行監査を正本とする。
- クラス存在だけでは1.0完了とせず、render queueの実出力、復旧導線、matte／mask順序、graph編集、初回workspace復元は受入れ確認待ち。

**判定:** この文書は優先順位の基準として有効だが、実装完了表ではない。現在は1-3／1-5／1-6／1-7の実経路確認を優先し、動画・外部plugin・高度な3Dは後段扱いを維持する。
