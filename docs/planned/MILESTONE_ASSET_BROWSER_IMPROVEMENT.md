> **SUPERSEDED** — 2026-08-04: 統合先 [MILESTONE_ASSET_BROWSER_IMPROVEMENT_2026-04-01.md](MILESTONE_ASSET_BROWSER_IMPROVEMENT_2026-04-01.md)
...
2026-03-27 時点で、Name / Type / Size / Date のソート切替を追加し、Phase 1 のソート基盤を前進させた。
2026-04-05 時点で、Favorites トグルと breadcrumb / history / sort status の見え方を強め、探索導線の実用度を上げている。
2026-04-05 時点で、Asset Browser ↔ Project View の選択同期をつなぎ、資産を見ながらプロジェクト側を追跡しやすくしている。
2026-04-09 時点で、Project View 側の footage 選択も Asset Browser に返すようにして、往復同期を実運用に近い形へ寄せている。
2026-04-09 時点で、Asset Browser 側にも Project View linked の sync chip を追加し、同期状態が見えるようにしている。
2026-08-15 時点で、コンテキストメニューの Add to Project も非同期 import 経路へ統一し、完了後の Undo 登録・一覧更新・プレビュー更新をコールバックへ移した。
2026-03-27 時点で、一覧の thumbnail 生成を遅延化し、初期表示時の重い同期処理を減らす方向に進めている。
...
| タスク | 対象ファイル | 見積 |
|---|---|---|
| ブラウザ ↔ Project View 選択同期 | `ArtifactAssetBrowser.cppm` + `ArtifactProjectManagerWidget.cppm` | 実装済み |
| 連動状態の chip 表示 | `ArtifactAssetBrowser.cppm` + `ArtifactProjectManagerWidget.cppm` | 実装済み |
...
| タスク | 対象ファイル | 見積 |
|---|---|---|
| 右ペインのメタデータ表示を owner-draw 化 | `ArtifactAssetBrowser.cppm` | 完了 |
| 右ペインの File Details を owner-draw 化 | `ArtifactAssetBrowser.cppm` | 完了 |
