...
drop-frameを29.97／59.94以外へ設定した場合の書き出し設定検証も追加。
FFmpeg出力ヘッダーへ開始タイムコードとdrop-frame表記（`;`）を出力する設定も接続。
...
実装済み: proxy URIのオンライン状態を保存・再検査し、原メディアがオフラインでも有効なproxyを使ったクリップ解決を可能にした。
...
`normalizeToTargetLufs()`で解析したセグメントへLUFS目標値のゲインを適用するAPIも追加。
