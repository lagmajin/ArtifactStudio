...
将来的には、エフェクトをレイヤー全面だけでなく `Rect` / `Mask` などの部分適用に対応させる前提も視野に入れる。
これにより、全体補正と局所補正を同じスタックで扱えるようにする。
mask を切らなくても effect 単体で範囲指定できる導線を先に用意し、局所補正の入口コストを下げる。
...
- 目的:
  - エフェクトの適用範囲をレイヤー全面以外にも広げる
  - `Rect` / `Mask` ベースの局所適用を扱えるようにする

- 作業項目:
  - effect context に region / matte / mask を持たせる
  - `Rect` と `Mask` の適用範囲モデルを分離する
  - global effect と local effect の優先順位を決める
  - UI に「どの範囲へ効いているか」を見せる
  - mask なしの簡易 region 指定から開始できる導線を追加する

- 完了条件:
  - 少なくとも `Rect` と `Mask` の部分適用が同じスタックで扱える
  - 全体適用と局所適用の差が UI で分かる
  - 将来の `Shape` / `Track Matte` 系に拡張できる
  - mask を切らずに effect 単体で範囲指定できる
