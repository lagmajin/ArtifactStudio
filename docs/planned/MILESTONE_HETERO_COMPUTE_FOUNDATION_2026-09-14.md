...
- 済（出力不変・既定不変）: (1) `FFmpegEncoderSettings.threadCount` を追加し `codecCtx_->thread_count` へ接続。未設定時は encoder default（=現行動作）。`ARTIFACT_XPU_ENCODER_THREADS` で上書き可（0〜64、0 は default）、`buildNativeVideoSettings`/`buildGpuVideoSettings` 共通。(2) 進捗 preview の時間間引き（既定 250ms、最終フレームは常時 publish）。`ARTIFACT_XPU_PREVIEW_MIN_INTERVAL_MS` で上書き可（0 で従来どおり、出力に影響しない）。(3a) 連番書込の bounded async（`ARTIFACT_XPU_ASYNC_SEQUENCE=on` または `ARTIFACT_XPU=asyncseq=on` で std::async＋ImageExporter、既定 off）。単チャンネル image sequence のみを並列書込し、pending 上限は `2*maxInFlightFrames_`、最終 drain で失敗を ledger へ反映。`asyncseq=manager` で `AsyncImageWriterManager` 経由（QImage→RawImage 変換＋boost thread_pool）に切替可。video/HTML/SVG/multi-channel/deep は同期のまま。(3b) 動画向け 3-stage pipeline の最小形（`ARTIFACT_XPU_PIPELINE=on` または `ARTIFACT_XPU=pipeline=on` でのみ有効、既定 off）。取得 serial→変換 parallel（RGBA8888 への detach を `std::async` で並列）→encode serial（`addFrame` は順序必須）の構造を consumer thread 内で実現。出力不変。(5) `maxInFlightFrames_` を `ARTIFACT_XPU_MAX_IN_FLIGHT` で可変化（既定 4=現行動作）。preset 露出は bench 用に `ARTIFACT_XPU_PRESET` / `ARTIFACT_XPU=preset=<value>` で opt-in 上書き可（未設定は `slow`/`p4` のまま、出力が変わるため既定では使わない）。
...
- 対象: `ArtifactCore/src/Image/FFmpegEncoder.cppm`、`ArtifactCore/include/Video/FFMpegEncoder.ixx`、`Artifact/src/Render/ArtifactRenderQueueEncoder.cppm`（(1)(5)＋preset override）、`Artifact/src/Render/ArtifactRenderQueueService.cppm`（(2)(3a)(3b)(5)、`xpuQImageToRawImage`/`AsyncImageWriterManager`）。
- Bench 手順（P3 有効範囲）: 4K png sequence（`asyncseq=on` vs `asyncseq=manager` vs off）と 4K h264 mp4（`threads=0/4/8/16` × `preset=slow/medium/fast`/`p4/p7`、pipeline on/off）で同一入力の wallMs と出力同一（hash）を比較。sws 律速は `threads` と `pipeline` の組合せで分離し、consumer 律速は `asyncseq` で分離する。
- DoD: 同一出力で wall-time 短縮、画質・仕様不変、4K 時の変換律速が緩和（未受入・実機 bench 要）。

### P4: iGPU assist 本格化＋適応調整 — 2026-09-23 部分実装（DoD 未達）

- 済（出力不変・opt-in/plan 予約）: `buildXpuPlan` で `igpu=assist` 時に `GpuAssist` ノードを列挙し、`xpuPlanDebugState` と job summary に出す。preview 縮小のみ `igpu=assist` 時に async（将来の iGPU offload 雛形）で実行し、full/off では同期のまま。device 生成は従来どおり直列・専有を維持。throughput は `xpuCpuMs`/`xpuGpuMs` を frame 毎に計測し、job summary に `gpuAvgMs`/`cpuAvgMs`/`weightGpu`/`weightCpu`（`gpuAvg/cpuAvg`）を出す（autotune の観測点、重みはまだ dispatch に反映しない）。NVENC/QSV 上限は `ARTIFACT_XPU_MAX_HW_ENCODERS`（既定 2）をログし、実 backoff（同時実行数に応じた CPU fallback）は encode セッション多重化時に `NativeFFmpegBackend::close` の decrement と合わせて実装予定で現状は挙動を変えない。
- 残: convert/scale/scope/proxy/encode 補助への本格接続、device 別 throughput の重みを dispatch へ反映（autotune）、NVENC/QSV 同時実行の実 backoff、PCIe 競合の backoff（失敗時は CPU/GPU-full へ fallback）。
