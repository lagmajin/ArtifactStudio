...
1. `Project vs 单層 Wipe` を比較系の入口にする
2. `マスク密度 / カバレッジ ヒートマップ` を単層向けの差分確認として足す
3. `3D マテリアル プレビュー Primitive` は比較表示の後段に置く
4. `アンカー / ピボット中心オーバーレイ` を review しやすい補助として追加する

### LW-3 完了条件

- 単層とプロジェクト結果をビューポート内で比較できる
- マスクの塗りむらを確認できる
- 3D 選択時の簡易プレビューが比較基盤に乗る
- アンカー中心が見える
