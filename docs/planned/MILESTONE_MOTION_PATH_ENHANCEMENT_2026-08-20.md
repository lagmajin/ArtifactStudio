...
- `Escape` でモーションパスの選択集合とホバー状態を解除できる。
- タンジェントの In / Out ホバーは拡大表示され、HUD に対象側とフレーム番号を表示する。
- キーホバーHUDには位置に加えて補間タイプを表示する。
- 選択中・ホバー中のキーには `F<frame>` ラベルを表示し、対象キーを直接識別できる。
- 複数選択時は選択範囲の破線バウンディングボックスと変形ピボットを表示する。
- 複数キーの回転・拡縮では位置キーと空間タンジェントを同じ変換で更新し、Bezier形状を保持する。
- タンジェントHUDは `Linked` / `Broken` の状態と、現在の操作ヒントを表示する。
- 連動タンジェントは青系、Broken Tangent はオレンジ系で描画し、状態を色でも識別できる。
