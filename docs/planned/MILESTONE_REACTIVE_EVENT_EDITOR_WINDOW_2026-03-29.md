...
- ルールはフレーム末キューで評価される
- キーフレームは編集対象にしない
- UI は `PropertyOverlay` を編集・可視化する
- 1 つの `ContactSubscription { triggerLayer, reactorLayer }` に複数の `Reaction` をぶら下げる
- `EventBus` は登録済み `TriggerPair` だけを処理する
...
- `TimelineReaction` は再生ヘッド制御の編集導線を優先する
