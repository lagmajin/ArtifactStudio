...
- 現行リポジトリでも `std`／Qt の混在は継続しているが、全体一括置換を実装着手点とはしない。
- `std::unique_ptr`／`std::function`／`std::optional`／`std::array` 等は用途上の標準型として維持し、Qt への機械的置換対象から除外する。
- C++20 modules、GPU／バッファ hot path、ABI／DLL 境界では、型置換よりも責務と依存の局所化を優先する。実装候補は、個別の Core wrapper と採用箇所を設計レビュー後に進める。
- 現段階では親リポジトリ側に安全な横断置換の実装単位はなく、Core 子リポジトリの基盤型・移行方針と同期して再開する。
