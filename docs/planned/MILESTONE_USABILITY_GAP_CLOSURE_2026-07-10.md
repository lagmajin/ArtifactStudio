...
| M-UG-4 | 式ピックwhip | Expression Copilot に `thisComp.layer("…")` と現在編集中プロパティの evaluator 対応参照 `value` をドラッグ挿入する参照リストを実装（2026-07-25 静的確認） | ✅ ソース実装完了（runtime 検証は未実施） |
...
`Text Animator N / Field` として読める。さらに Animator グループには `Anim` バッジを付け、
通常のプロパティグループと視覚的に区別する。ただしアニメータ追加、セレクタ専用行、
スタックの折りたたみを持つ専用パネルは未実装のため、本項目は未完了のままとする。
