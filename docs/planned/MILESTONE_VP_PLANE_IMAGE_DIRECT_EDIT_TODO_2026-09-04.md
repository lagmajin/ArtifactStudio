...
### F6. 参照画像の位置・倍率・不透明度調整

- **状態:** 実装予定・未着手（2026-09-17 ユーザー依頼で記録）。矩形指定の部分プレビューを優先し、本項目は後続とする。
- **内容:** 下絵用の参照画像を移動・拡縮し、不透明度を調整できるようにする。初期値へ戻す操作も用意する。
- **既存経路:** `Artifact/src/Widgets/Render/ArtifactCompositionEditor.cppm` の参照画像読み込みと、`ArtifactCompositionRenderController.cppm` の `setReferenceOverlayImage`／参照画像描画。現行ソースでは中央フィット・不透明度 `0.38` 固定。
- **制約:** レイヤーの transform や出力画像を変更しない表示補助とする。既存の GPU 描画・入力経路を再利用し、操作のフォーカスとローカルバインドを守る。表示状態の保存先は着手時に決める。
- **確認:** pan／zoom／リサイズ後も指定した配置を保つこと、表示解除・初期化、編集対象レイヤーとの入力競合がないこと、出力結果に参照画像が混入しないこと。
- **優先度／規模:** 中／小〜中。実機検証は未実施。

### 2026-09-17 の優先順位

1. ユーザーが選好した **矩形指定の部分プレビュー（Interactive ROI）** を次の優先候補とする。表示範囲由来の自動 ROI と区別し、任意矩形の指定・変更・解除、範囲外表示、エフェクトの周辺参照領域、キャッシュ無効化を設計する。現時点では未実装で、速度改善は未計測。
2. 後続の実装予定は F6 と [比較表示の完成](MILESTONE_COMPOSITION_EDITOR_SELECTION_COMPARISON_2026-06-27.md)。Diff／固定参照フレームの描画を先に成立させ、F4 の Wipe 拡張と重複しないようにする。
3. 以下の旧着手順より、この優先順位を優先する。今回の更新は計画記録のみで、実装・ビルド・起動は行わない。
